...
# If the rolling standard deviation is smaller than this, treat it as "flat"
# and skip the z-score division instead of dividing by (near) zero.
...
def compute_features(service_name: str, metric_name: str, value: float) -> dict:
    ...
    if len(history) >= 2:
        ...
        if std > Z_SCORE_MIN_STD:
            ...
            z_score = 0.0  # Std is basically flat — avoid dividing by ~0
    else:
